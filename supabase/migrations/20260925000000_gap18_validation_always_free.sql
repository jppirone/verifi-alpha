-- Gap #18 (2026-09-25, John's explicit product requirement): validation is entirely free, for every
-- candidate, every tier, always. Payment has zero relationship to whether or what gets validated. The
-- ONLY thing ever monetized is choosing to EXCLUDE a specific item from a specific delivery stream (the
-- generated PDF/export) — free-tier candidates cannot exclude anything, everything they have flows
-- through by default; paid candidates get the option to leave something out.
--
-- This reverses one specific piece of the 2026-09-21 customization rebuild's own decision (see that
-- migration's header: "paid-only, overrides go DORMANT on downgrade and return on re-upgrade") — NOT the
-- whole paid-gating design. The candidate's ability to EDIT text, or to EXPLICITLY EXCLUDE an item via an
-- override, stays paid-gated exactly as before (v_apply is unchanged). What changes is narrower: the
-- DEFAULT for freeform's "included" flag, which used to silently exclude anything section_type =
-- 'needs_review' UNLESS a paid override explicitly turned it back on. Investigated first (see gap #8 —
-- confirmed this was deliberate, working-as-designed behavior, not a bug) before reversing it now.
--
-- Two changes, both scoped to the "other freeform sections" block only — every other category
-- (work/education/certifications/skills) already defaulted to included=true regardless of tier, untouched:
--   1. 'included' default: coalesce(o.included, f.section_type <> 'needs_review') -> coalesce(o.included, true).
--      A paid candidate's existing override (explicit include OR exclude) still wins when present — only the
--      fallback when no override exists changes, matching every other category's own default already.
--   2. A 'verification' block is now attached to every freeform item, all four section_types uniformly
--      (summary/hobbies_other/additional_info/needs_review alike — none of these are ever independently
--      checked against an outside source, same as a Skill), status 'not_checked' — the EXACT SAME
--      candidate-safe vocabulary value _cz_vstatus already emits elsewhere for "no verification_items row
--      exists at all" (see that function, migration 20260921090000). Reused deliberately, not a new term:
--      John's own instruction was "we will just put in the JSON file that this is unverified data," and
--      'not_checked' already means exactly that everywhere else in this output. Before this, freeform items
--      carried no verification field at all — a real, confirmed gap, not a stylistic omission.
