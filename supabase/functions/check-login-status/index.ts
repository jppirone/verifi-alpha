...
export default {
  fetch: withSupabase({ auth: "none" }, async (req, _ctx) => {
    ...
    try {
      ...
      let candidate: { email?: string; phone?: string; full_name?: string; first_name?: string; last_name?: string; deletion_scheduled_at?: string | null; tier?: string; tour_completed_at?: string | null; header_display_mode?: string; personal_location?: string | null } = {};
      if (record.candidate_id) {
        const candRes = await fetch(
          `${SUPABASE_URL}/rest/v1/candidates?id=eq.${record.candidate_id}&select=email,phone,full_name,first_name,last_name,deletion_scheduled_at,tier,tour_completed_at,header_display_mode,personal_location`,
          { headers: { "apikey": SUPABASE_SERVICE_ROLE_KEY, "Authorization": `Bearer ${SUPABASE_SERVICE_ROLE_KEY}` } },
        );
