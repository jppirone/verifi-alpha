...
async function sendGuestLink(requestId: string): Promise<boolean> {
  ...
  const ok = await sendEmail(
    ...
    `<p>Hi ${esc(r.requester_name || "there")},</p><p>The candidate approved your request. ${lr ? "What they shared is a license status report: each license on their account and whether it is verified." : "What they shared is a comparison of their verified record with your copy."}</p><p><a href="${SITE}/employer.html?comparison=${token}">${lr ? "Open your license status report" : "Open your comparison"}</a></p><p>This is a <b>one-time view</b>. Nothing is charged until you choose to open it: opening costs ${esc(money)}, paid on Stripe's own page. The moment your payment is confirmed it opens by itself and you can read it for 30 minutes, so have your own copy ready before you pay. If you pay and leave before it appears, open this same link again and it appears then. After the 30 minutes it is gone and cannot be reopened, by you or by us. There is no account: this link is the only way in, so keep this email. The link works until ${esc(new Date(r.snapshot_expires_at).toUTCString())} if you do not open it.</p>`,
  );
