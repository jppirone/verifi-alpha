...
// identity), no correction_* fields (staff-internal correction bookkeeping unrelated to any
// candidate-facing flow, since no candidate-side override/dispute mechanism exists yet — see this
// task's investigation), and never the raw internal `status` value itself. Raw status stays
// server-side in this response; candidate.html's own safe-label allowlist (see
// verificationSafeLabel) is what ever reaches the rendered page, so there's still a second,
...
export default {
  fetch: withSupabase({ auth: "none" }, async (req, _ctx) => {
    ...
    try {
      ...
      const url = SUPABASE_URL + "/rest/v1/verification_items?select=id,type,claim,status,created_at&candidate_id=eq."
