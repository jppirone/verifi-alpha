...
sometimes reports a short fragment — often just one or two words, commonly ending in a comma — as its
OWN separate line, sitting between two other lines that plainly form one continuous sentence together
once that fragment is removed from between them. A real, confirmed example: OCR text read "...adoption
playbooks for SMB and enterprise clients" / "Automate," / "across ChatGPT, Claude, Gemini, Jasper, and
Microsoft Power accelerating time-to-value for each" / "engagement." — the isolated fragment "Automate,"
had visually broken away from its real position (immediately after "Microsoft Power," completing the
product name "Microsoft Power Automate,") and was reported one line too early, corrupting a real
product name ("Microsoft Power" instead of "Microsoft Power Automate") and starting the next clause
with an orphaned word ("Automate across..." instead of "across..."). When a short (1-3 word) OCR line
is grammatically incomplete on its own, does not read as a genuine new bullet or sentence of its own,
and the lines immediately before and after it read as one continuous sentence that is ITSELF
grammatically incomplete or broken without that fragment restored to it — reconstruct the sentence in
its natural reading order (the fragment belongs wherever it completes a term or clause correctly,
almost always immediately before the specific word it visually separated from) rather than transcribing
the OCR's own arbitrary line placement verbatim. This is never a license to rewrite or improve wording
that already reads correctly as printed — it only undoes a clear, mechanical OCR misordering where a
short fragment visibly landed on the wrong line. When genuinely unsure whether a short line is a real,
intentional fragment (an actual short bullet or clause of its own) or a misplaced piece of the
surrounding sentence, leave it exactly as OCR reported it rather than guessing.
...
and holds even when a page break falls in the middle of the list of sub-entries. Only a line that is
...
// scoped to companies with header lines visible on THIS SAME page (a role split across a page boundary
// stays the separate continuation-merge step's job, not this one). A company appears in at most one.
...
const BOUNDARY_SCHEMA_SHAPE = `{
  "sections": [
    { "heading": string, "category": "work_history" | "education" | "certifications" | "skills" | "summary" | "hobbies_other" | "additional_info" | "unknown", "is_continuation_of_previous_page": boolean }
  ],
...
is styled more lightly than the page's other headings — smaller, not bold, or Title Case where every
other heading on the page is ALL CAPS. Do not let a lightly-styled heading get read as trailing content
of the section before it just because the list under it happens to share the same shape (e.g. both are
...
appears more than once ON THIS SAME PAGE within whatever section(s) you judged as work_history above,
whether it must merge into one work_history entry or stay as separate entries. Decide it definitively
here, for every repeated company on this page, so nothing is left ambiguous.

The test: count the COMPLETE header lines naming this company on this page. A complete header line is
one that, by itself, states a title, this company, AND a date range (the same header shape the page
uses for every other employer entry) — not a parenthetical or a subordinate aside underneath a single
shared header.
  - Exactly ONE complete header line names this company on this page, with any other role for it
    mentioned ONLY as a subordinate aside UNDER that one header (a parenthetical, an italicized note, a
    short line like "Promoted mid-year from Desktop Publishing Manager (1996-1997)" with no company/
    location/date-range fields of its own) → list the company's exact printed name in
    "mergeCompanies". This is a genuine internal promotion/title-change within one tenure.
  - TWO OR MORE complete header lines independently name this company on this page, each with its own
    title AND its own date range → list the company's exact printed name in "noMergeCompanies",
    regardless of whether the roles read like an internal promotion or carry a caption such as "Role
    transition" or "Promoted from...". A caption on one header never erases that the other role has its
    own separate, complete header elsewhere.
  - The company's two mentions read as genuinely separate, disconnected stints with an unrelated gap
    → list it in neither array; leave it out entirely.
A company appears in at most one of the two arrays, never both. When no company on this page repeats
at all, return two empty arrays.
...
function resolveContinuationCategory(boundaries: BoundaryResult, previousPageContext?: TrailingItemContext | null): BoundaryResult {
  ...
  if (previousPageContext.kind === "work_history") {
    ...
    else if (previousPageContext.sectionType === "additional_info") resolvedCategory = "additional_info";
  }
...
"Projects"/"Portfolio"/"Career Highlights"/"Workplace Strengths"/"Professional Affiliations"/"Awards"/
...
same fallback described in "THE ONE EXCEPTION" above, just applied to the whole page rather than one
flagged section, since no per-section decision exists this time. A header that matches none of the
above goes to needs_review, same as always.`;
  ...
  // the skills_secondary case above — scoped to this page only. The merge-eligible direction turned
  // out to need this just as much as the don't-merge direction (see extract-resume-fields's own header
  // comment on this same field for the full story of why both are needed, not just one).
...
the following employer names have two or more independent, complete header lines on THIS PAGE — each
...
have exactly one complete header line on this page, with any other role for that same tenure mentioned
only as a subordinate aside underneath it — combine each into ONE work_history entry per the HOW TO
MERGE steps below (joined title, earliest start_date, latest end_date, full job_responsibilities
including the aside's own content):
