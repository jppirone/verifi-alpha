...
below, since it governs how to apply every one of them): when a standalone, left-aligned line on
THIS page is functioning as a section header — judge this by its own visual role on the page (a
short line set apart from body content, never itself part of a sentence or list item), never by
matching it against an exact string — and that header SEMANTICALLY reads as one of the known
categories (work_history, education, certifications, skills, summary, hobbies_other), every item
beneath it on THIS page, up to the next header, belongs to that category. The header's own literal
wording never decides this: "Certifications," "Professional Certifications," "AI & Emerging
Technology Certifications," "Credentials," or even an unconventional label like "Clown
Certifications" all semantically read as a certifications header and route their content to
certifications the same way — judge the CONCEPT the header names, never its exact phrasing. Same
for every other category: "Skills," "Core Competencies," "Technical Skills," "Areas of Expertise"
all semantically mean skills; "Experience," "Work History," "Employment History," "Professional
Experience" all semantically mean work_history; "Education," "Academic Background" both
semantically mean education. This header match is deliberately the PRIMARY signal — it overrides
what the content's own shape might otherwise suggest. A section headed "Certifications" whose
entries don't happen to include a license number, or a section headed "Skills" whose items read
more like short phrases than single terms, still classify by their header, not by second-guessing
the header against the content's shape. (Real, confirmed failure mode this closes: a real 4-item
"Professional Certifications" list, correctly heading-labeled, was still classified entirely as
skills because its items' own shape looked more skill-like than the certifications definition's
usual "name (issuer)" pattern — the header alone should have settled it.)

SHAPE-BASED CLASSIFICATION IS THE FALLBACK, FOR TWO SPECIFIC CASES ONLY (do not apply it more
broadly than these two): (1) a plain, minimally-formatted page with no section headings at all, no
bold text, no visual separation whatsoever — real work history, education, and certifications on a
page like this must still be extracted into their real structured categories, judged by their own
...
identifies certifications); and (2) a single section on THIS page genuinely has no dividing header
between two different kinds of content running together — see the CERT/LICENSE VS. SKILL
DISAMBIGUATION rule below, now itself a fallback for exactly this shape of case, not a general-
purpose rule. Outside these two cases, never let a section's content shape override or second-guess
a header that already gave a clear semantic match — the header wins. Never let the mere absence of
a heading push content that otherwise clearly fits a real category (by shape, case 1) into
needs_review — that catch-all is for content that genuinely doesn't fit any category by either
signal, not for content that fits one perfectly but happens to lack a visible label.
...
- CERT/LICENSE VS. SKILL DISAMBIGUATION — A FALLBACK FOR THE HEADER RULE'S OWN BLIND SPOTS, not a
  ...
  own shape-based fallback covers — (a) a single section on this page contains a genuine MIX of
  cert-shaped and skill-shaped items with no dividing header separating them, or (b) the mixed
  content has no section header at all on this page to classify by. In either case, use the presence
  or absence of a discernible trailing identifier as the signal to split the section: items
...
- skills = whatever sits under a header on THIS page that semantically reads as Skills (e.g.
  "Skills," "Core Competencies," "Technical Skills," "Areas of Expertise," "Key Skills," or similar —
  see CLASSIFICATION IS HEADER-DRIVEN above, the header is the primary signal now). Route everything
  ...
  order. Only fall back to judging by SHAPE (a flat list of short terms/phrases rather than
  sentences) when there is genuinely no header at all on this page to classify by, per the
  shape-based fallback rule above — in that headerless case only, content under NO heading that
  reads as a list of short terms belongs in skills, and content under no heading that's actually
  prose belongs elsewhere (summary, or needs_review). Don't duplicate the same term into skills and
  any other category.
  ...
  This holds even when THIS page (or an earlier page) also has a genuinely skills-shaped section
  (e.g. "Core Competencies") — a second bulleted list elsewhere is NOT automatically more of the same
  skills block just because its individual phrases look similar; check each item for its own
  explanatory clause before adding anything to skills in the headerless case. If THIS page
